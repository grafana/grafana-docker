#!/bin/bash
_grafana_version=$1
_grafana_tag="$2-alpine"
_release_build=false

if [ -z "${_grafana_version}" ]; then
	source GRAFANA_VERSION_ALPINE
	_grafana_version=$GRAFANA_VERSION
	_grafana_tag="$GRAFANA_VERSION-alpine"
	_release_build=true
fi
...
if [ $_release_build == true ]; then
	docker tag "adipirro852/grafana:${_grafana_tag}" "adipirro852/grafana:alpine"
fi
